...
public class RecordingService extends Service implements MediaRecorder.OnInfoListener {
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        switch (action) {
            case ACTION_START:
                doStartRecording(intent.getIntExtra(EXTRA_AUDIO_SOURCE, 0), intent.getBooleanExtra(EXTRA_SHOW_TAPS, false), intent.getBooleanExtra(EXTRA_LOW_QUALITY, false), intent.getBooleanExtra(EXTRA_SHOW_STOP_DOT, false));
                break;
    ...
    protected void doStartRecording(int audioSource, boolean showTaps, boolean lowQuality, boolean showStopDot) {
        int mCurrentUserId = mUserContextTracker.getCurrentUserContext().getUserId();
    ...
    protected void doStartRecording(int audioSource, boolean showTaps) {
      doStartRecording(audioSource, showTaps, false, false);
    }
